...
from load_images import loadBGBoard, getHomeButton, getEndGameButton
...
def getDrawPlayerPositions(self, numPlayers):
    ...
    botDY = 260; topDY = 80; leftDX = 50; rightDX = 50
    result = []

    for i in range(len(playerList)):
        if i % 2 == 0:
            x = self.width / 2 - cardWidth
            if i == 0: y = self.height - botDY
            else: y = topDY
        else:
             y = self.height / 2 - 3*offset
             if i == 1: x = leftDX
             else: x = self.width - rightDX - cardWidth
        result.append((x,y))
    return result
...
def drawPlayerCards(self, screen):
    ...
    for (i, player) in enumerate(playerList):
        for (j, card) in enumerate(player.hand[:2]):
            (x,y) = cardPos[i]
